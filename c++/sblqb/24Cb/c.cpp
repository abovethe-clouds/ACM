...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        cin >> a[i].fir.x >> a[i].fir.y >> a[i].sec.x >> a[i].sec.y;
    }
    int ans=0;
    for (int i=0; i < n; i++)
    {
        for (int j=0;j<i;j++)
        {
            set<pt> st1 = inters(a[i].fir, a[i].sec, a[j].fir, a[j].sec);
            if (st1.size()>0)
            {
                if (st1.size()==1)
                {
                    pt jiao=*st1.begin();
                    pt ii={static_cast<double>(static_cast<int>(jiao.x)),static_cast<double>(static_cast<int>(jiao.y))};
                    if ((onSegment(a[i].fir,a[i].sec,ii)&&onSegment(a[j].fir,a[j].sec,ii))||(ii==a[i].fir||ii==a[i].sec))
                    {
                        st.insert(ii);
                    }
                }
